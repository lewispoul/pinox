        ...
        if database_pool:
            try:
                async with database_pool.acquire() as conn:
                    ...
                    status_info["database"]["pool_size"] = len(database_pool._queue._queue) if hasattr(database_pool, '_queue') else 0
            except Exception as e:
...
async def get_prometheus_metrics():
    ...
    try:
        ...
        if database_pool:
            ...
            metrics.append(f"nox_database_connections {len(database_pool._queue._queue) if hasattr(database_pool, '_queue') else 0}")
