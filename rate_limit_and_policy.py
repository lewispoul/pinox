...
class RateLimitAndPolicyMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, policies_file: str = None):
        ...
        self.daily_quotas = defaultdict(lambda: {"cpu_seconds": 0, "requests": 0, "last_reset": time.time()})
    ...
    def get_default_policies(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "shell_policy": {
                "mode": "blacklist",
                "forbidden_commands": [
                    "rm", "reboot", "shutdown", "mkfs", "dd", "mount", "umount", 
    ...
    def create_audit_log(self, request: Request, response: Response, 
                        token: str, start_time: float, error: str = None):
                ...
                try:
                    ...
                    if body:
                        ...
                        if not self.validate_shell_command(command):
                            ...
                            self.create_audit_log(request, response, token, start_time, f"forbidden_command: {command}")
                            return response
                except Exception:
