...
st.set_page_config(
    page_title="Nox API v2.3 - Dashboard Admin",
    page_icon="🚀",
    layout="wide"
)
...
def main_dashboard():
    ...
    with col1:
        ...
        st.caption(f"Connecté en tant que: **{st.session_state.user_info.get('email')}** ({st.session_state.user_info.get('role')})")
...
def show_files_tab():
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader("Choisir un fichier", type=['py', 'txt', 'json', 'yaml', 'md'])
...
def show_python_tab():
    ...
    if st.button("▶️ Exécuter Python", type="primary"):
        try:
            ...
            st.info(f"Code de retour: {result.get('returncode')} | Utilisateur: {result.get('user')}")
            ...
            for line in lines:
                if line and not line.startswith('#'):
                    ...
                    if len(parts) >= 2:
                        ...
                        if any(keyword in metric_name for keyword in ['nox_requests', 'nox_latency', 'sandbox']):
