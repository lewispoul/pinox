...
async def lifespan(app: FastAPI):
    ...
    # Startup
    database_url = os.getenv("DATABASE_URL", "postgresql://noxuser:test_password_123@localhost:5432/noxdb")
...
@app.get("/ls")
def ls(path: str = ".", authorization: str | None = Header(default=None, alias="Authorization")):
...
def cat(path: str, authorization: str | None = Header(default=None, alias="Authorization")):
    ...
    try:
        ...
        return {"content": f"<binary file, {len(content_bytes)} bytes>", "file_path": path}
...
if __name__ == "__main__":
    ...
    print(f"   - Audit: Enabled with session tracking")
    print(f"   - Admin: /admin/audit/* endpoints available")
    print(f"   - Export: CSV/JSON capabilities enabled")
