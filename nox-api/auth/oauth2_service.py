...
class OAuth2Service:
        ...
        async with self.pool.acquire() as conn:
            ...
            if existing_user:
                ...
                await conn.execute(
                    ...
                    provider, oauth_id, email, datetime.utcnow()
                )
            ...
            if existing_email_user:
                ...
                await conn.execute(
                    """UPDATE users 
                       ...
                       WHERE email = $1""",
                    email, provider, oauth_id, datetime.utcnow()
                )
                return User(**dict(existing_email_user))
            
            # Create new user
            user_id = uuid.uuid4()
            hashed_password = pwd_context.hash(f"oauth_{provider}_{oauth_id}")  # Placeholder password
            ...
            await conn.execute(
                ...
                user_id, email, hashed_password, UserRole.USER.value, True,
                provider, oauth_id, datetime.utcnow(), datetime.utcnow()
            )
    ...
    async def authenticate_oauth_user(
        ...
        try:
            ...
            if provider == 'google':
                ...
                if not email:
                    return None, "GitHub email not available. Please make your email public."
            else:
    ...
    def create_jwt_tokens(self, user: User) -> Dict[str, Any]:
        ...
        access_token = jwt.encode(
            access_token_data, self.jwt_secret, algorithm="HS256"
        )
