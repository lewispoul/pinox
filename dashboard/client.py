...
class NoxClient:
    ...
    def put(self, dest_path: str, local_path: str):
        with open(local_path, "rb") as f:
            files = {"f": f}
            r = self.session.post(f"{self.base_url}/put", params={"path": dest_path}, files=files, timeout=self.timeout)
        r.raise_for_status()
        return r.json(), r.headers

    def run_py(self, code: str, filename: str="run.py"):
        r = self.session.post(f"{self.base_url}/run_py", json={"code": code, "filename": filename}, timeout=self.timeout)
        r.raise_for_status()
        return r.json(), r.headers
    ...
    def delete_file(self, path: str):
        r = self.session.delete(f"{self.base_url}/delete", params={"path": path}, timeout=self.timeout)
        r.raise_for_status()
        return r.json(), r.headers
