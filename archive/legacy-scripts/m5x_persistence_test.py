...
class PersistenceValidator:
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(f"{self.base_url}/quotas/my/usage", headers=self.headers, timeout=5)
    ...
    def compare_states(self, before: Dict[str, Any], after: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in before["quotas"]:
            if key in after["quotas"]:
                ...
                quota_differences[key] = {"before": before["quotas"][key], "after": "missing"}
        ...
        return {
            ...
            "usage_preserved_or_increased": all(
                ...
                for key in before["usage"] 
                if key != "updated_at" and key != "user_id" and isinstance(before["usage"][key], (int, float))
            )
        }
    ...
    def run_persistence_test(self) -> Dict[str, Any]:
        ...
        if initial_state["status"] != "success":
            return {"test_result": "failed", "step": 1, "reason": "Could not get initial state", "data": initial_state}
        
        print(f"   ✅ Initial state captured")
        print(f"   📊 Usage: {initial_state['usage']['req_hour']} req/hour, {initial_state['usage']['req_day']} req/day")
        ...
        print(f"   ✅ Activity generated: {activity['successful_requests']}/{activity['total_requests']} successful")
        ...
        if post_activity_state["status"] != "success":
            return {"test_result": "failed", "step": 3, "reason": "Could not get post-activity state"}
        
        print(f"   ✅ Post-activity state captured")
        print(f"   📊 Usage: {post_activity_state['usage']['req_hour']} req/hour, {post_activity_state['usage']['req_day']} req/day")
        ...
        if not restart_success:
            return {"test_result": "failed", "step": 4, "reason": "Could not restart API server"}
        ...
        if post_restart_state["status"] != "success":
            return {"test_result": "failed", "step": 5, "reason": "Could not get post-restart state", "data": post_restart_state}
        
        print(f"   ✅ Post-restart state captured")
        print(f"   📊 Usage: {post_restart_state['usage']['req_hour']} req/hour, {post_restart_state['usage']['req_day']} req/day")
        ...
        if comparison["comparison"] == "failed":
            return {"test_result": "failed", "step": 6, "reason": "State comparison failed", "data": comparison}
        
        # Step 7: Results analysis
        print(f"\n📊 Persistence Test Results:")
        print(f"   🎯 Quotas preserved: {'✅ Yes' if comparison['quotas_preserved'] else '❌ No'}")
        print(f"   📈 Usage preserved: {'✅ Yes' if comparison['usage_preserved_or_increased'] else '❌ No'}")
        
        if comparison["quota_differences"]:
            print(f"   ⚠️  Quota differences found:")
            for key, diff in comparison["quota_differences"].items():
                print(f"      {key}: {diff['before']} → {diff['after']}")
        
        if comparison["usage_differences"]:
            print(f"   📈 Usage differences found:")
...
def main():
    ...
    if result["test_result"] == "passed":
        ...
        print(f"   📋 Database successfully preserves quota and usage data across API restarts")
    else:
        print(f"   ❌ PERSISTENCE TEST FAILED")
        print(f"   📋 Issue at step {result.get('step', 'unknown')}: {result.get('reason', 'unknown')}")
