...
class IntelligentPolicyEngine:
    ...
    async def evaluate_access_request(self, request: AccessRequest) -> PolicyDecision:
        ...
        try:
            ...
            decision_mapping = {0: AccessDecision.DENY, 1: AccessDecision.ALLOW, 2: AccessDecision.CHALLENGE}
            ...
            final_decision, risk_factors = await self._apply_risk_based_adjustments(
                primary_decision, decision_confidence, risk_level, risk_confidence, request
            )
    ...
    async def _extract_request_features(self, 
                                      request: AccessRequest, 
                                      user_profile: UserAccessProfile) -> Optional[List[float]]:
        ...
        try:
            ...
            resource_familiarity = 1.0 if request.resource in user_profile.typical_resources else 0.0
    ...
    async def _apply_risk_based_adjustments(self, 
                                          primary_decision: AccessDecision,
                                          decision_confidence: float,
                                          risk_level: int,
                                          risk_confidence: float,
                                          request: AccessRequest) -> Tuple[AccessDecision, List[str]]:
    ...
    async def _get_applicable_rules(self, 
                                  request: AccessRequest, 
                                  user_profile: UserAccessProfile) -> List[str]:
    ...
    def _evaluate_rule_condition(self, 
                               condition: str, 
                               request: AccessRequest, 
                               user_profile: UserAccessProfile) -> bool:
    ...
    async def _determine_challenge_requirements(self, 
                                              request: AccessRequest,
                                              risk_level: int,
                                              user_profile: UserAccessProfile) -> Dict[str, Any]:
                ...
                with conn.cursor() as cursor:
                    cursor.execute(
                        ...
                        (request.user_id, request.resource, request.action,
                         decision.decision.value, decision.confidence_score,
                         json.dumps(decision.applied_rules), decision.reasoning,
                         json.dumps(decision.risk_factors), request.timestamp,
            ...
            for i, prob in enumerate(role_probs):
                if prob > 0.1:  # Only include roles with reasonable probability
                    recommendations.append({
                        ...
                        "confidence": prob,
                        "reasoning": self._generate_role_reasoning(role_names[min(i, len(role_names)-1)], user_profile)
                    })
    ...
    def _extract_role_features(self, user_profile: UserAccessProfile) -> Optional[List[float]]:
        ...
        try:
            ...
            admin_resource_count = sum(1 for res in user_profile.typical_resources if "admin" in res.lower())
