...
class OAuth2Profile:
    ...
    def __init__(self, provider: str, provider_user_id: str, email: str,
                 name: str = None, username: str = None, avatar_url: str = None):
    ...
    def from_microsoft(cls, user_info: dict) -> 'OAuth2Profile':
        ...
        return cls(
            ...
            username=user_info.get("mailNickname") or user_info.get("userPrincipalName", "").split("@")[0],
...
class EnhancedOAuth2Service:
    ...
    async def store_oauth2_tokens(self, user_id: str, provider: str, 
                                  access_token: str, refresh_token: str = None,
                                  expires_in: int = 3600, scope: str = None) -> str:
        """Store OAuth2 tokens in database"""
        token_id = str(uuid.uuid4())
        expires_at = datetime.utcnow() + timedelta(seconds=expires_in)
        refresh_expires_at = datetime.utcnow() + timedelta(days=self.refresh_token_expire_days) if refresh_token else None
    ...
    async def refresh_oauth2_token(self, token_id: str, client_ip: str = None, 
                                   user_agent: str = None) -> Optional[OAuth2TokenPair]:
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                WHERE id = $4
                """,
                new_access_token, new_refresh_token, new_expires_at, token_id
            )
    ...
    async def store_oauth2_profile(self, user_id: str, profile: OAuth2Profile, 
                                   raw_profile_data: dict) -> None:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                profile.avatar_url, json.dumps(raw_profile_data),
    ...
    async def create_or_update_oauth2_user(self, profile: OAuth2Profile, 
                                           raw_profile_data: dict) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            if existing_user:
                ...
                await conn.execute(
                    ...
                    """,
                    user_id, profile.email, "", profile.provider, profile.provider_user_id,
                    100, 3600, 512, 1024  # Default quotas for OAuth2 users
                )
    ...
    async def revoke_user_oauth2_tokens(self, user_id: str, provider: str = None) -> int:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.fetchval(
                "SELECT revoke_oauth2_tokens($1, $2)",
                user_id, provider
            )
            return result
...
oauth2_service = EnhancedOAuth2Service(
    database_url=f"postgresql://noxuser:test_password_123@localhost:5432/noxdb",
    jwt_secret=f"nox-oauth2-secret-change-in-production"
)
